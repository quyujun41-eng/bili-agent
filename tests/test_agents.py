...
mock 全部外部依赖（LLM、SQL、RAG、router、memory），不发任何真实请求
"""
import json
import pytest
...
def _sql_error(msg="DB连接失败"):
    return json.dumps({"error": msg, "sql": "SELECT ?"})

def _rag_results(n=3):
    return [
        {"id": i, "title": f"推荐视频{i}", "author": "UP主",
         "partition": "搞笑", "year": 2024, "score": 0.9 - i*0.1, "hybrid": True}
        for i in range(n)
    ]
...
@patch("agents._get_router")
@patch("agents.sql_query_tool")
def test_sql_agent_chunk_sequence(mock_tool, mock_router, mock_hist, mock_add):
    """sql_agent 应按 memory → agent → text… → done 顺序输出"""
    mock_tool.invoke.return_value = _sql_ok()
    mock_router.return_value.invoke.return_value = {"agent": "sql_agent"}
    # 禁用缓存
    with patch("agents._cache_get", return_value=None):
        with patch("agents._cache_set"):
            with patch("agents._stream_llm", return_value=iter(["视频A排第一", "，播放量最高"])):
                import agents
                chunks = list(agents.sql_agent_stream("播放量最高的视频", "s1"))

    types = [c["type"] for c in chunks]
    assert "memory" in types, "应有 memory chunk"
    assert "agent"  in types, "应有 agent chunk"
...
@patch("agents._get_router")
@patch("agents.sql_query_tool")
def test_sql_agent_done_has_correct_fields(mock_tool, mock_router, mock_hist, mock_add):
    mock_tool.invoke.return_value = _sql_ok(5)
    mock_router.return_value.invoke.return_value = {"agent": "sql_agent"}
    with patch("agents._cache_get", return_value=None), patch("agents._cache_set"):
...
@patch("agents._get_router")
@patch("agents.sql_query_tool")
def test_sql_empty_result_returns_no_data_text(mock_tool, mock_router, mock_hist, mock_add):
...
def test_rag_agent_done_has_rag_fields(mock_rag, mock_router, mock_hist, mock_add):
    mock_router.return_value.invoke.return_value = {"agent": "rag_agent"}
    with patch("agents._rewrite_query", return_value="推荐搞笑视频"):
...
def test_rag_agent_no_results_returns_text(mock_rag, mock_router, mock_hist, mock_add):
    """RAG 检索无结果时，应输出 text 提示"""
    mock_router.return_value.invoke.return_value = {"agent": "rag_agent"}
    with patch("agents._rewrite_query", return_value="查不到的视频主题"):
        import agents
...
@patch("agents.mem.get_history", return_value=[])
@patch("agents._get_router")
def test_chat_agent_no_sql_no_rows(mock_router, mock_hist, mock_add):
    mock_router.return_value.invoke.return_value = {"agent": "chat_agent"}
...
# ── 错误回退（SQL 报错 → fallback chat_agent）────────────────────────────────

@patch("agents.mem.add_turn")
@patch("agents.mem.get_history", return_value=[])
@patch("agents._get_router")
@patch("agents.sql_query_tool")
def test_sql_error_emits_error_chunk(mock_tool, mock_router, mock_hist, mock_add):
    """SQL 报错时应包含 error chunk"""
...
@patch("agents._get_router")
@patch("agents.sql_query_tool")
def test_sql_error_fallback_still_returns_done(mock_tool, mock_router, mock_hist, mock_add):
...
@patch("agents.mem.get_history", return_value=[])
@patch("agents._get_router")
def test_cache_hit_returns_cached_answer(mock_router, mock_hist, mock_add):
    """Redis 缓存命中时，直接返回缓存内容，不调用 LLM"""
    mock_router.return_value.invoke.return_value = {"agent": "sql_agent"}
    cached = {
        "answer": "缓存的回答", "sql": "SELECT 1",
        "columns": ["id"], "rows": [{"id": 1}], "total": 1, "chart": "bar",
    }
    with patch("agents._cache_get", return_value=cached):
        with patch("agents._stream_llm") as mock_llm:
            import agents
            chunks = list(agents.sql_agent_stream("缓存测试", "s9"))
    mock_llm.assert_not_called()
    text_chunks = [c for c in chunks if c["type"] == "text"]
    assert any("缓存的回答" in c.get("text", "") for c in text_chunks)
...
@patch("agents.mem.get_history", return_value=[])
@patch("agents._get_router")
def test_memory_add_turn_called_on_success(mock_router, mock_hist, mock_add):
    ...
    assert args[0] == "sess_mem"
    assert args[1] == "问题"
    assert "回答内容" in args[2]


@patch("agents.mem.add_turn")
@patch("agents.mem.get_history", return_value=[])
@patch("agents._get_router")
def test_no_session_id_skips_memory(mock_router, mock_hist, mock_add):
    """不传 session_id 时，不应调用 mem.add_turn"""
    mock_router.return_value.invoke.return_value = {"agent": "chat_agent"}
    with patch("agents._stream_llm", return_value=iter(["回答"])):
        import agents
        list(agents.sql_agent_stream("问题"))
