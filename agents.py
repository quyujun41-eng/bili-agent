...
def _cache_get(question: str):
    ...
    try:
        import redis as _r
        r = _r.from_url(config.REDIS_URL, decode_responses=True, socket_connect_timeout=2, socket_timeout=2)
        raw = r.get(key)
        if raw:
...
def _stream_llm(system: str, messages: list, max_tokens: int = 400) -> Iterator[str]:
    if config.LLM_PROVIDER == "anthropic":
        ...
        stream = client.chat.completions.create(model=config.OPENAI_MODEL, max_tokens=max_tokens,
...
def sql_agent_stream(question: str, session_id: str = "default",
    ...
    if agent == "sql":
        ...
        if not history:
            _cache_set(question, {"agent": "sql", "answer": answer, "sql": sql,
                                  "columns": cols, "rows": rows[:50], "total": len(rows), "chart": chart})
    ...
    # ── RAG agent ─────────────────────────────────────────────────────────────
        ...
        ctx     = "\n".join([f"- 《{r['title']}》 作者：{r['author']} 分区：{r['partition']}" for r in results])
        answer  = ""
        for text in _stream_llm(
